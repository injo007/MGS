...
import { Download, Inbox, LineChart, Mail, RefreshCw, Server, Target, TrendingUp } from "lucide-react";
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    fetch("/api/servers?pageSize=500&sortBy=name&sortOrder=asc")
      .then((res) => {
      ...
      .then((json) => {
        ...
        setServers(rows);
        if (rows[0]?.id) setTrackingServerId(rows[0].id);
      })
      .catch(() => setServers([]));
  }, []);
